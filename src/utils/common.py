...
----------------
格式为：
拍号：请从 '4/4'、'6/8'、'3/4'中选择一个
速度：表示一分钟内击奏的节拍数，请用数字表示，如'120'表示一分钟演奏120个节拍
接下来是正式演奏的部分，每行代表一个四分音符的时间的演奏：
请用字母'A'表示军鼓，字母'B'表示中高音嗵鼓，字母'C'表示中低音嗵鼓，字母'D'表示落地嗵鼓，字母'E'表示闭音踩镲，字母'F'表示开音踩镲，
字母'G'表示低音大鼓，字母'H'表示高音吊镲，字母'I'表示中音吊镲，字母'J'表示叮叮镲，字母'O'表示休止符。
每个字母后面跟着一个下划线和一个数字，数字表示在这个四分音符的时间内演奏的次数。
...
E_2 G_2
A_1 E_2
表示速度为 120 的一小节鼓谱，拍号是 4/4 拍。
第一行表示一个四分音符时间内演奏高音吊镲一次，闭音踩镲两次，低音大鼓一次。
